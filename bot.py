...
class ConfigCommands(commands.Cog):
    ...
    @discord.app_commands.command(name="clear_all_users", description="Clear all verification records and remove the verified role from all users.")
    @is_admin()
    async def clear_all_users(self, interaction: discord.Interaction):
        """Slash command to clear all verification records and remove the verified role from all users."""
        await interaction.response.defer(ephemeral=True)  # Defer the response to avoid timeouts
        try:
            # Fetch the server configuration
            config = await get_config(interaction.guild.id)
            if not config or not config.verified_role_id:
                embed = discord.Embed(
                    ...
                    description="The verified role is not configured for this server.",
                    color=discord.Color.red(),
                )
                await interaction.followup.send(embed=embed, ephemeral=True)
                return

            # Remove all verification records from the database
            async with async_session() as session:
            ...
            # Remove the verified role from all users
            verified_role = interaction.guild.get_role(config.verified_role_id)
            removed_count = 0
            if verified_role:
            ...
            if config.log_channel_id:
                ...
                if log_channel:
                    embed = discord.Embed(
                        title="All Verification Records Cleared",
                        description=(
                            f"All verification records were cleared by {interaction.user.mention}.\n"
                            f"Cleared {count} records and removed the verified role from {removed_count} users."
                        ),
                        color=discord.Color.orange(),
                    )
                    embed.add_field(name="Cleared By", value=interaction.user.mention, inline=True)
            ...
            embed = discord.Embed(
                ...
                description=(
                    f"Successfully cleared {count} verification records.\n"
                    f"Removed the verified role from {removed_count} users."
                ),
                color=discord.Color.green(),
            )
            await interaction.followup.send(embed=embed, ephemeral=True)
        except Exception as e:
            print(f"Error in /clear_all_users: {e}")
            embed = discord.Embed(
                title="Error",
                description="An error occurred while processing the command. Please try again later.",
                color=discord.Color.red(),
